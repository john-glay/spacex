...
export default function BodyList() {
  ...
  const { getAllList } = bindActionCreators(actionList, useDispatch());
    ...
    <div key={item.id}>
      <div className="base">
        ...
        <div className="descriptions">
            <div className="title"><h3 className="number">{`${item.flight_number} :`}</h3>
          <p>{`${item.name}`}</p>
          <p>{`( ${item.date_utc.substring(0, 10)} )`}</p></div>
